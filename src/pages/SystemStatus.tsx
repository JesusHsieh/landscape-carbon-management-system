...
const SystemStatus = () => (
  <div className="space-y-6 animate-in fade-in duration-500 font-sans">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={i} className="bg-white/5 p-4 rounded-2xl border border-white/5 space-y-1">
                <span className="text-[13px] text-secondary font-bold uppercase tracking-widest">{stat.label}</span>
                <div className="flex flex-col">
                  <span className={cn("text-lg font-black leading-tight", stat.color || "text-ink")}>{stat.val}</span>
                  <span className="text-[13px] text-secondary font-medium">{stat.sub}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 group hover:border-primary/30 transition-all">
                ...
                <span className={cn("text-[13px] font-bold", item.done ? "text-primary" : "text-secondary/40")}>{item.status}</span>
              </div>
            ))}
          </div>
        </Card>
            ...
            {[
              ...
              <div key={i} className="flex gap-4 items-start relative pb-4 last:pb-0">
                ...
                <div className={cn("w-8 h-8 rounded-xl border-2 flex items-center justify-center shrink-0 z-10", step.active ? "border-primary bg-primary/20 text-primary" : "border-white/5 bg-white/5 text-secondary/40")}>
                  <span className="text-[13px] font-black">{step.v}</span>
                </div>
                <div className="flex flex-col pt-1">
                  <span className={cn("text-[13px] font-bold", step.active ? "text-ink" : "text-secondary/60")}>{step.t}</span>
                  {step.v === 'v1.0' && <span className="text-[13px] text-primary/60 italic">ISO 對齊式碳管理報告架構</span>}
                </div>
              </div>
            ))}
          </div>
        ...
        <Card title="Prototype Disclaimer｜原型限制聲明" className="border-accent/20 bg-accent/5">
          <div className="flex gap-4">
            ...
            <div className="space-y-2">
              <p className="text-[13px] text-accent font-bold leading-relaxed">
    ...
    <div className="p-6 bg-white/5 rounded-3xl border border-white/5 space-y-4">
      <h4 className="text-[13px] font-black text-secondary tracking-widest uppercase">功能區域分工</h4>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="space-y-2">
          <div className="text-[13px] font-bold text-primary">1. 右上角全域操作區</div>
          <p className="text-[13px] text-secondary leading-relaxed">用於搜尋、通知與使用者狀態，不負責系統管理。</p>
        </div>
        <div className="space-y-2">
          <div className="text-[13px] font-bold text-primary">2. 右下角快速設定</div>
          <p className="text-[13px] text-secondary leading-relaxed">用於語言、單位、顯示模式與圖表細節，不改變資料與計算邏輯。</p>
        </div>
        <div className="space-y-2">
          <div className="text-[13px] font-bold text-primary">3. 系統狀態頁</div>
          <p className="text-[13px] text-secondary leading-relaxed">用於顯示原型成熟度、資料成熟度、方法論成熟度與未來 Roadmap，不提供後台管理功能。</p>
        </div>
      </div>
    </div>
  </div>
